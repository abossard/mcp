...
[CommandMetadata(
    Id = "cd0094ee-7dc7-4e57-acc5-0018033a96b0",
    Name = "create",
    Title = "Create Signal Definition",
    Description = """
    ...
    Secret = false,
    LocalRequired = false)]
public sealed class SignalDefinitionCreateCommand(ILogger<SignalDefinitionCreateCommand> logger, IHealthModelsService service, ISubscriptionResolver subscriptionResolver)
    : SubscriptionCommand<SignalDefinitionCreateOptions, HealthModelsItemResult>(subscriptionResolver)
{
    private readonly ILogger<SignalDefinitionCreateCommand> _logger = logger;
    private readonly IHealthModelsService _service = service;

    public override async Task<CommandResponse> ExecuteAsync(CommandContext context, SignalDefinitionCreateOptions options, CancellationToken cancellationToken)
    {
        try
        {
